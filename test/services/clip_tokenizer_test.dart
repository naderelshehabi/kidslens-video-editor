...
void main() {
  ...
  group('ClipBuiltInTokens.getPreTokenized', () {
    final knownPrompts = <String, Int32List>{
      ...
      'woman in revealing clothing': ClipBuiltInTokens.womanInRevealingClothing,
    ...
    for (final entry in knownPrompts.entries) {
      ...
      test('returned value for "${entry.key}" matches the static constant', () {
        final result = ClipBuiltInTokens.getPreTokenized(entry.key);
    ...
    test('returns null for unknown prompt', () {
      final result = ClipBuiltInTokens.getPreTokenized('random unknown prompt');
