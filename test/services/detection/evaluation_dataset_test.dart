...
void main() {
  group('EvaluationDataset', () {
    test('family safety smoke dataset covers every required fixture kind', () {
      final dataset = EvaluationDataset.familySafetyV1Smoke;
      ...
      expect(
        dataset.clips.expand((clip) => clip.groundTruth).map((truth) => truth.categoryId),
        containsAll(const [
          'blood',
          'explicit_nudity',
          'gore',
  ...
  group('EvaluationBox', () {
    ...
    test('rejects boxes outside normalized frame', () {
      ...
      expect(box.validate(), contains('box must remain within normalized frame'));
    });
  });
